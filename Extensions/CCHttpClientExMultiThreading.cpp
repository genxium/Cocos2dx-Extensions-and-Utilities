//
//  CCHttpClientExMultiThreading.cpp
//  ScrollTest
//
...
static CCHttpClientExMultiThreading *s_pHttpClient = NULL; // pointer to singleton
...
static void* singleTaskThread(void *data){
    do {
        ...
        switch (request->getRequestType())
        {
            ...
            default:
                CCAssert(true, "CCHttpClientExMultiThreading: unkown request type, only GET and POSt are supported");
                break;
        }
...
bool configureCURL(CURL *handle)
{
    ...
    code = curl_easy_setopt(handle, CURLOPT_TIMEOUT, CCHttpClientExMultiThreading::getInstance()->getTimeoutForRead());
    if (code != CURLE_OK) {
        return false;
    }
    code = curl_easy_setopt(handle, CURLOPT_CONNECTTIMEOUT, CCHttpClientExMultiThreading::getInstance()->getTimeoutForConnect());
    if (code != CURLE_OK) {
        return false;
    }
...
CCHttpClientExMultiThreading* CCHttpClientExMultiThreading::getInstance()
{
    if (s_pHttpClient == NULL) {
        s_pHttpClient = new CCHttpClientExMultiThreading();
    }
    
    return s_pHttpClient;
}

void CCHttpClientExMultiThreading::destroyInstance()
{
    CCAssert(s_pHttpClient, "");
    CCDirector::sharedDirector()->getScheduler()->unscheduleSelector(schedule_selector(CCHttpClientExMultiThreading::dispatchResponseCallbacks), s_pHttpClient);
    s_pHttpClient->release();
}

CCHttpClientExMultiThreading::CCHttpClientExMultiThreading()
: _timeoutForConnect(30)
, _timeoutForRead(60)
{
    CCDirector::sharedDirector()->getScheduler()->scheduleSelector(
                                                                   schedule_selector(CCHttpClientExMultiThreading::dispatchResponseCallbacks), this, 0, false);
...
void CCHttpClientExMultiThreading::send(CCHttpRequest* request)
{
...
void CCHttpClientExMultiThreading::dispatchResponseCallbacks(float delta)
{
...
bool CCHttpClientExMultiThreading::isHalted(){
    bool ret=false;
#if kUseTryLock == 1
    if(0==pthread_mutex_trylock(&s_bHaltMutex)){
#else
